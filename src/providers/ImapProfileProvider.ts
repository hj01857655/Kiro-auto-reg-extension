...
export class ImapProfileProvider {
  ...
  private async _syncFromSettings(): Promise<void> {
    ...
    try {
      ...
      const port = config.get<number>('imap.port', 993);
      ...
      if (activeProfile) {
        ...
        console.log('[ImapProfileProvider] Created profile from VS Code settings');
      }
    } catch (err) {
      console.error('[ImapProfileProvider] Failed to sync from settings:', err);
    } finally {
      this._syncingFromSettings = false;
    }
  }
